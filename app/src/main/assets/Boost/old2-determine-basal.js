    ...
    var now = new Date().getHours();
        ...
        if (tdd_pump < (0.5 * tdd7)){
          ...
          //  }
            //else {
            //TDD = ( tdd_pump * 0.7 );
            //console.error("TDD = 0.5 x pump TDD due to no 7 day average. TDD = "+TDD+"; ");
            //rT.reason += "TDD = 0.5 x pump TDD due to no 7 day average. TDD = "+TDD+"; ";
            //}
...
/* var profile_sens = round(profile.sens,1);
    ...
    console.error("UAM Impact:",uci,"mg/dL per 5m; UAM Duration:",UAMduration,"hours");
        ...
        if( glucose_status.delta >= 0 && meal_data.mealCOB > 0) {
...
// Start of TS experimental closed loop code to enable scaling of SMBs to increase insulin early in glucose rise
                ...
                //With COB, allow a large initial bolus
                 ...
                 else if ( now1 >= boost_start && now1 < boost_end && COB > 0 && lastCarbAge < 40  ){
                       var cob_boost_max = Math.max((( COB / CR ) / insulinReqPCT),boost_max);
                 ...
                 else if (COB = 0 && glucose_status.delta >= 5 && glucose_status.short_avgdelta >= 3 && uamBoost1 > 1.2 && uamBoost2 > 2 && now1 >= boost_start && now1 < boost_end && iob_data.iob < boostMaxIOB && boost_scale < 5 && eventualBG > target_bg && bg > 80 && insulinReq > 0  /*&& target_bg < 82*/) {
                 ...
                 else if ( glucose_status.delta > 8  && iob_data.iob < boostMaxIOB && now1 >= boost_start && now1 < boost_end && eventualBG > 108 && COB = 0) { /*|| eventualBG >
                  180 && bg > 162 && iob_data.iob < boostMaxIOB && now1 > boost_start && now1 < boost_end*/
                    if (insulinReq > boostMaxIOB-iob_data.iob) {
                       insulinReq = boostMaxIOB-iob_data.iob;
                       }
                       else {
                 ...
                 else if (bg > 108 && glucose_status.delta > 3 && eventualBG > target_bg && iob_data.iob < boostMaxIOB && now1 >= boost_start && now1 < boost_end && COB = 0 ) {
                      if (insulinReq > boostMaxIOB-iob_data.iob) {
                          insulinReq = boostMaxIOB-iob_data.iob;
                      }
                      else {
                 ...
                 //Use standard SMB calculation for overnight periods and when none of the criteria above apply
                   ...
                   console.error("Insulin required % ("+((1/insulinReqPCT) * 100)+"%) applied.");
                   rT.reason += "Standard oref1 operating";
                }
