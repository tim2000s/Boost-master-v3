...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, isSaveCgmSource) {
    ...
    var iTimeMax = (( new Date(systemTime).getTime() - Math.min(meal_data.firstBolusCorr, meal_data.firstCarbTime )) / 60000);
...
//    enlog += "HypoPredBG:"+HypoPredBG+";\n";
    ...
    if( glucose_status.delta >= 0) {
        ...
        sens_future = sens_normalTarget / (Math.max(eventualBG,2.2)/normalTarget); // safety * EXPERIMENT *
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            var UAMBoostReason = (!eatingnowtimeOK ? "EN: inactive" : "EN: no boost"); //reason text for oaps pill is nothing to start
            UAMBoostReason = (!iTimeOK ? "EN: not iTime" : "EN: no boost"); //reason text for oaps pill is nothing to start
