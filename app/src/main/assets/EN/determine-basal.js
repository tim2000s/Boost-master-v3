...
//    }
    ...
    csf = sens_normalTarget / profile.carb_ratio; // test for slow carb absorption
    ...
    var assumedCarbAbsorptionRate = 25; // g/h; maximum rate to assume carbs will absorb if no CI observed *JAMIE*
...
//    enlog += "Experimental test, EBG:"+EBG+",eventualBG:"+eventualBG+"\n";
//    enlog += "HypoPredBG:"+HypoPredBG+";\n";
    ...
    // limit sens_future to ISF_Max and if limited set sens_future_max to true for reason
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            var UAMBoostReason = (!eatingnow ? "EN: inactive" : "EN: no boost"); //reason text for oaps pill is nothing to start
            UAMBoostReason = (!iTimeOK ? "EN: no iTime" : "EN: no boost"); //reason text for oaps pill is nothing to start
            ...
            if (eatingnow && eventualBG > normalTarget && iTimeOK) {
                ...
                if (UAMBoostOK && UAMBoost > UAMBoost_threshold) {
                    ...
                    //EatingNowMaxCOBSMB = (COBBoostOK || (meal_data.mealCOB > 0 && cTime < 30) && !ignoreCOBPatch ? (meal_data.mealCOB / profile.carb_ratio) * profile.EatingNowPrebolusPct : EatingNowMaxCOBSMB);
                    EatingNowMaxCOBSMB = (COBBoostOK ? (meal_data.mealCOB / profile.carb_ratio) * profile.EatingNowPrebolusPct : EatingNowMaxCOBSMB);
                ...
                if (profile.ISFBoost_enabled && !UAMBoosted && eventualBG > target_bg) {
                    ...
                    //EatingNowMaxCOBSMB = (COBBoostOK || (meal_data.mealCOB > 0 && cTime < 30 && !ignoreCOBPatch) ? (meal_data.mealCOB / profile.carb_ratio) * profile.EatingNowPrebolusPct : EatingNowMaxCOBSMB);
                    EatingNowMaxCOBSMB = (COBBoostOK ? (meal_data.mealCOB / profile.carb_ratio) * profile.EatingNowPrebolusPct : EatingNowMaxCOBSMB);
                    insulinReqPct = (COBBoostOK ? 1 : insulinReqPct); // 100% insulinReqPct with COB in the first 15 mins, preBolusPct still applies
