...
class ENFragment : DaggerFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        openapsma_run.setOnClickListener {
            ENPlugin.invoke("EatingNow button", false)
        }
    }
    ...
    fun updateGUI() {
        ...
        ENPlugin.lastDetermineBasalAdapterENJS?.let { determineBasalAdapterENJS ->
            openapsma_glucosestatus.text = JSONFormatter.format(determineBasalAdapterENJS.glucoseStatusParam)
            openapsma_currenttemp.text = JSONFormatter.format(determineBasalAdapterENJS.currentTempParam)
            try {
            ...
            openapsma_profile.text = JSONFormatter.format(determineBasalAdapterENJS.profileParam)
            openapsma_mealdata.text = JSONFormatter.format(determineBasalAdapterENJS.mealDataParam)
            openapsma_scriptdebugdata.text = determineBasalAdapterENJS.scriptDebug
            ENPlugin.lastAPSResult?.inputConstraints?.let {
                openapsma_constraints.text = it.getReasons(aapsLogger)
            }
        }
        if (ENPlugin.lastAPSRun != 0L) {
            openapsma_lastrun.text = dateUtil.dateAndTimeString(ENPlugin.lastAPSRun)
        }
