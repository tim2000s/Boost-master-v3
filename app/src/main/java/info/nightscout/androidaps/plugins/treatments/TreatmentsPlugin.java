...
//            getAapsLogger().debug(MedtronicHistoryData.doubleBolusDebug, LTag.DATATREATMENTS, "DoubleBolusDebug: FilteredTreatments: AfterTime={}, Items={} " + fromTimestamp + " " + new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create().toJson(in5minback));
    ...
    public long getLastENBolusTime(String boostType) {
        ...
        if (last == null) {
            getAapsLogger().debug(LTag.DATATREATMENTS, "Last EN " + boostType + " bolus time: NOTHING FOUND");
            return 0;
        } else {
            getAapsLogger().debug(LTag.DATATREATMENTS, "Last EN " + boostType + " bolus time: " + dateUtil.dateAndTimeString(last.date));
            return last.date;
        }
    }
