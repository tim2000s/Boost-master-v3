...
@Singleton
public class AIMIPlugin extends PluginBase implements APSInterface, ConstraintsInterface {
    ...
    // last values
    DetermineBasalAdapterAIMIJS lastDetermineBasalAdapterAIMIJS = null;
    long lastAPSRun = 0;
    DetermineBasalResultAIMI lastAPSResult = null;
    ...
    public void invoke(String initiator, boolean tempBasalFallback) {
        ...
        lastAPSResult = null;
        DetermineBasalAdapterAIMIJS determineBasalAdapterAIMIJS;
        determineBasalAdapterAIMIJS = new DetermineBasalAdapterAIMIJS(new ScriptReader(context), getInjector());
        ...
        IobTotal[] iobArray = iobCobCalculatorPlugin.calculateIobArrayForSMB(lastAutosensResult, AIMIDefaults.exercise_mode, AIMIDefaults.half_basal_exercise_target, isTempTarget);
        ...
        try {
            determineBasalAdapterAIMIJS.setData(profile, maxIob, maxBasal, minBg, maxBg, targetBg, activePlugin.getActivePump().getBaseBasalRate(), iobArray, glucoseStatus, mealData,
        ...
        DetermineBasalResultAIMI determineBasalResultAIMI = determineBasalAdapterAIMIJS.invoke();
        profiler.log(LTag.APS, "SMB calculation", start);
        if (determineBasalResultAIMI == null) {
            getAapsLogger().error(LTag.APS, "SMB calculation returned null");
            lastDetermineBasalAdapterAIMIJS = null;
            lastAPSResult = null;
            lastAPSRun = 0;
        } else {
            // TODO still needed with oref1?
            // Fix bug determine basal
            if (determineBasalResultAIMI.rate == 0d && determineBasalResultAIMI.duration == 0 && !treatmentsPlugin.isTempBasalInProgress())
                determineBasalResultAIMI.tempBasalRequested = false;

            determineBasalResultAIMI.iob = iobArray[0];

            try {
                determineBasalResultAIMI.json.put("timestamp", DateUtil.toISOString(now));
            } catch (JSONException e) {
                getAapsLogger().error(LTag.APS, "Unhandled exception", e);
            }

            determineBasalResultAIMI.inputConstraints = inputConstraints;

            lastDetermineBasalAdapterAIMIJS = determineBasalAdapterAIMIJS;
            lastAPSResult = determineBasalResultAIMI;
            lastAPSRun = now;
        }
