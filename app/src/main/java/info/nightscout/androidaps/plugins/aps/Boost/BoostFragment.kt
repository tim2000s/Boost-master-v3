...
class BoostFragment : DaggerFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        openapsma_run.setOnClickListener {
            boostPlugin.invoke("Boost button", false)
        }
    }
    ...
    fun updateGUI() {
        ...
        boostPlugin.lastDetermineBasalAdapterBoostJS?.let { determineBasalAdapterBoostJS ->
            openapsma_glucosestatus.text = JSONFormatter.format(determineBasalAdapterBoostJS.glucoseStatusParam)
            openapsma_currenttemp.text = JSONFormatter.format(determineBasalAdapterBoostJS.currentTempParam)
            try {
            ...
            openapsma_profile.text = JSONFormatter.format(determineBasalAdapterBoostJS.profileParam)
            openapsma_mealdata.text = JSONFormatter.format(determineBasalAdapterBoostJS.mealDataParam)
            openapsma_scriptdebugdata.text = determineBasalAdapterBoostJS.scriptDebug
        ...
        if (boostPlugin.lastAPSRun != 0L) {
            openapsma_lastrun.text = dateUtil.dateAndTimeString(boostPlugin.lastAPSRun)
        }
