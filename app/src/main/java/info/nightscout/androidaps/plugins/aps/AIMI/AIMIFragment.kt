...
class AIMIFragment : DaggerFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        openapsma_run.setOnClickListener {
            AIMIPlugin.invoke("AIMI button", false)
        }
    }
    ...
    fun updateGUI() {
        ...
        AIMIPlugin.lastDetermineBasalAdapterAIMIJS?.let { determineBasalAdapterSMBJS ->
        ...
        if (AIMIPlugin.lastAPSRun != 0L) {
            openapsma_lastrun.text = dateUtil.dateAndTimeString(AIMIPlugin.lastAPSRun)
        }
